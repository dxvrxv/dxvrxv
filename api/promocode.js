    ...
    if (req.method === "POST") {
        ...
        req.on("end", async () => {
            try {
                ...
                // });
                res.end(encrypt(JSON.stringify({ data: { present: JSON.stringify(atob(data).split('|').map(item => item.split('-').map((part, index, arr) => isNaN(part) ? part : Number(part)))) }, result: 1 }), key));
            } catch (error) {
                console.error(error);
                res.statusCode = 500;
